...
term2table <- function(rowterm, colterm, env, n) {
  ...
  for (i in seq_along(allargs)) {
    ...
    if (is.call(e) && (fn <- deparse(e[[1]])) == ".Format")
      ...
      justification <- as.character(e[[if (length(e) > 2)
        3
        else
          2]])
    else if (fn == "Percent") {
      ...
      env1$Equal <- env1$Unequal <- function(...)
        sys.call()
      env1$Percent <- function(denom = "all", fn = percent) {
        if (is.null(summary)) {
          if (identical(denom, "all"))
            summary <<- function(x)
              fn(x, values)
          else if (identical(denom, "row"))
            summary <<- function(x)
              fn(x, values[rowsubset])
          else if (identical(denom, "col"))
            summary <<- function(x)
              fn(x, values[colsubset])
          else if (is.call(denom) &&
                   deparse(denom[[1]]) %in% c("Equal", "Unequal")) {
            summary <<- local({
      ...
      if (is.null(arguments))
        ...
        stop(gettextf(
          "Duplicate Arguments list: %s and %s",
          deparse(arguments),
          deparse(e)
        ))
    } else if (fn == "DropEmpty") {
      env1 <- new.env(parent = env)
      env1$DropEmpty <- function(empty = "",
                                 which = c("row", "col", "cell")) {
        good <- which %in% c("row", "col", "cell")
        if (!all(good))
          stop(gettextf(
            "bad 'which' value(s):  %s in %s",
            paste0("'", which[!good], "'", collapse = ","),
            deparse(e)
          ),
          call. = FALSE)
      ...
      if (asis ||
          is.vector(arg) || inherits(arg, "labelledSubset")) {
        if (missing(n))
          n <- length(arg)
  ...
  if (!is.null(pctdenom)) {
    # We need a second pass to find the subsets
    for (i in seq_along(allargs)) {
      e <- allargs[[i]]
      fn <- ""
      if (is.call(e))
      ...
      if (!(fn %in% c(".Format", "Justify", "Percent", "Heading"))
        ...
        if (!asis && is.logical(arg)) {
          if (inherits(arg, "labelledSubset")) {
            ...
            else if (pctdenom[1] == "Unequal" &&
                     argexpr %in% pctdenom[-1])
              pctsubset <- pctsubset | !arg
          } else
  ...
  if (missing(n))
    stop(gettextf(
      "Length of %s ~ %s is indeterminate",
      deparse(rowterm),
      deparse(colterm)
    ))
  if (is.null(summary)) {
    if (!is.null(arguments))
      stop(gettextf(
        "'%s specified without summary function",
        deparse(arguments)
      ))
    summary <- length
  }
  if (is.null(values) && is.null(arguments))
    values <- rep(NA, n)
  ...
  if (is.null(arguments))
    ...
    if (!is.null(values)) {
      ...
      for (i in rev(seq_along(arguments)[-1])) {
        ...
        if (named)
          names(arguments)[i + 1] <- names(arguments)[i]
      }
  ...
  structure(
    ...
    dropcell = ifelse(dropcell &&
                        !any(subset), empty, NA_character_),
...
moveColnames <- function(labels, do_move = (names != "")) {
  ...
  for (i in rev(seq_along(do_move))) {
    if (do_move[i]) {
      ...
      labels <- cbind(labels[, before, drop = FALSE], "", labels[, after, drop =
                                                                   FALSE])
      labels[1, i] <- names[i]
      ...
      if (length(justification))
        justification <- cbind(justification[, before, drop = FALSE], NA_character_, justification[, after, drop =
                                                                                                     FALSE])
      if (length(dropcell))
        dropcell <- cbind(dropcell[, before, drop = FALSE], NA_character_, dropcell[, after, drop =
                                                                                      FALSE])
    }
  }
...
getLabels <- function(e,
  ...
  Heading <- head
  result <- if (rows)
    matrix(NA, ncol = 0, nrow = 1)
  else
    matrix(NA, ncol = 1, nrow = 0)
  ...
  if (is.call(e) && (op <- deparse(e[[1]])) == "*")  {
    ...
    suppress <- rightsuppress
    neardata <- leftnearData &
      rightnearData # neardata=FALSE is needed for Hline
    ...
    if (rows) {
      ...
      if (is.null(leftnames))
        leftnames <- rep("", ncl)
      rightnames <- colnames(rightLabels)
      if (is.null(rightnames))
        rightnames <- rep("", ncr)
      ...
      if (ncl < ncr) {
        ...
        padNAmat <- matrix(NA_character_,
                           NROW(rightjustification),
                           abs(ncr - ncl))
        if (rightnearData) {
      ...
      if (!is.null(rightjustification))
        justification[j, k] <- rightjustification
      if (!is.null(head)) {
        result <- rbind(rep(NA_character_, ncol(result)), result)
        result[1, 1] <- head
    ...
    env1 <- new.env(parent = env)
    env1$Heading <- function(name = NULL,
                             override = TRUE,
      ...
      if (missing(name))
        ...
        if (!is.logical(override) || is.na(override))
          stop(
            gettextf(
              "'%s' argument in '%s' must be TRUE or FALSE",
              "override",
              deparse(e)
            ),
            call. = FALSE
    ...
    # The rest just add a single label; suppress it
    suppress <- suppress - 1
...
sumofprods <- function(e) {
  ...
  if (is.name(e))
    ...
    chr <- deparse(e[[1]])
    if (chr == "+")
      result <- c(sumofprods(e[[2]]), sumofprods(e[[3]]))
    else if (chr == "*") {
      left <- sumofprods(e[[2]])
      ...
      result <- sumofprods(e[[2]])
    else if (chr == "~")
      stop("Nested formulas not supported")
    else
      result <- list(e)
...
tabledims <- function(e) {
  ...
  if (length(result) > 2)
    stop("Only 2 dim tables supported")
  result
}
...
tabular.formula <- function(table,
                            data = NULL,
                            n,
                            suppressLabels = 0,
                            ...) {
  ...
  if (length(list(...)))
    warning(gettextf(
      "extra argument(s) %s will be disregarded",
  ...
  if (length(dims) == 1)
    dims <- c(list(quote((` ` = 1))), dims)
  dims[[1]] <- expandFactors(dims[[1]], data)
  rlabels <- getLabels(dims[[1]],
                       rows = TRUE,
...
latexNumeric <- function(chars,
  ...
  if (leftpad && any(ind <- leadin != ""))
    leadin[ind] <- paste("\\phantom{", gsub(" ", "0", leadin[ind]), "}", sep =
                           "")
  if (rightpad && any(ind <- tail != ""))
    tail[ind] <- paste("\\phantom{", gsub(" ", "0", tail[ind]), "}", sep =
                         "")
  if (mathmode)
