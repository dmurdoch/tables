...
\subsubsection{\code{Factor()} and \code{RowFactor()}}
\label{sec:RowFactor}
...
with more flexibility in the formatting.  They have syntax

\begin{Schunk}
\begin{Sinput}
Factor(x, name, levelnames)
RowFactor(x, name, levelnames, spacing=3, space=1, nopagebreak="\\nopagebreak")
\end{Sinput}
\end{Schunk}
...
\begin{center}
<<results=tex>>=
latex( tabular( Factor(Species, 
            levelnames=paste("\\multicolumn{3}{l}{\\em Iris", 
                            levels(iris$Species), 
                            "} \\\\ ")) * (mean + sd)  
            ~ All(iris), data=iris, suppress=1))
@
\end{center}
