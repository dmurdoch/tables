...
@
You can also pass the output through the \code{Hmisc::latex()} function \citep{Hmisc}
...
<<eval=FALSE>>=
booktabs(justification = "c")
@
for the rest of this document.  With a few modifications to the table formula (shown below in 
section \ref{sec:booktabs}), this gives
\begin{center}
<<irisbook, echo=FALSE, results=tex>>=
save <- booktabs(justification = "c")
latex(
tabular( Justify("l")*(Species + 1)*Justify("c") ~ (n=1) + Format(digits=2)*
         (Sepal.Length + Sepal.Width)*(mean + sd), data=iris )
...
method:
\begin{description}
\item[justification] Any justification string will be accepted; it should be
...
in the table.  By default these are all \code{"\\hline"}.
...
\item[pad]
Whether to pad cases that have leading blanks with spacing matching a digit width per space.
If \code{TRUE}, leading blanks will be converted to spaces the same width as a digit 0.  (If
\code{minus=TRUE}, one leading blank may have been consumed in the sign padding.)
Note that trailing blanks won't be modified.
...
Note that only one value vector can be specified for any cell in the table, and all value vectors must be the same length, or an error will be
reported.
...
<<results=tex>>=
stderr <- function(x) sd(x)/sqrt(length(x))
latex( tabular( Justify(l)*(Species+1) ~ All(iris)*
          PlusMinus(mean, stderr, digits=1), data=iris ) )
@
\end{center}

\subsubsection{\code{RowFactor()}}
\label{sec:RowFactor}

This function produces multiple rows the way a factor does, but 
with more flexibility.  It has syntax

\begin{Schunk}
\begin{Sinput}
RowFactor(x, name, spacing=3, space=1)
\end{Sinput}
\end{Schunk}
...
\item[name] The name to be used for the factor; by default, the name passed as \code{f}.
\item[spacing] Extra spacing is added after every \code{spacing} lines.
\item[space] How much extra space to add (in ``ex'' units).
\end{description}

Example: Show the first 50 lines of the iris dataset, in groups of 5 lines.  Use
the \code{"longtable"} environment to allow this to cross page boundaries.  The
\code{midrule} setting is necessary to get the headings to repeat on subsequent pages.  
I've used the one that is compatible with the \pkg{booktabs} style; if you want the
default style, use \verb!midrule="\\hline\\endhead\\hline\\endfoot"! instead.

<<results=tex>>=
subset <- 1:50
save <- table_options(tabular="longtable",
                      midrule="\\midrule\\endhead\\hline\\endfoot")
