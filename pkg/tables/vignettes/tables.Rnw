...
To add extra space after each top level group in a two-way
classification, use \code{spacing = 1} in the
top level factor.  For example:
\begin{center}
<<results=tex>>=
latex( tabular( RowFactor(dose, name="Dose", spacing=1, space=0.5)
                * Factor(supp, name="Supplement")  
                ~ (Length = len)*mean, data=ToothGrowth) )
@
\end{center}
