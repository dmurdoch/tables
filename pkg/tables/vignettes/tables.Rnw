...
Currently several examples of formula functions are provided, but not all are
...
<<results=tex>>=
subset <- 1:50
latex( tabular( Heading(i)*(Literal("\\endhead\\hline\\endfoot%") 
                 + RowFactor(subset, spacing=5))  ~ 
