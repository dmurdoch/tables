...
\item[\code{factorsAsStrings}] Whether factors should be converted to character vectors in the display
\end{description}
...
\begin{center}
<<results=tex>>=
latex( tabular( Species ~ All(iris[,-5])*mean, data=iris) )
@
\end{center}
...
<<results=tex>>=
stderr <- function(x) sd(x)/sqrt(length(x))
latex( tabular( Justify(l)*(Species+1) ~ All(iris[,-5])*
...
Example: Show the first 30 lines of the iris dataset, in groups of 5 lines.

<<results=tex>>=
latex( tabular( RowFactor(seq_len(30), "", 5)  ~ 
       All(iris[1:30,])*Heading()*identity )) 
@
