...
If you prefer the style of table that the \LaTeX\ \pkg{booktabs} package produces, you can choose
that style:
\begin{center}
...
For example, the earlier table of iris data was produced using
<<eval=FALSE>>=
<<irisbook>>
@
\begin{center}
...
save <- table_options(tabular="longtable",
                      midrule="\\hline\\endhead\\hline\\endfoot")
