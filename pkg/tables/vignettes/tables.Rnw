...
\item[options] A list of options to pass to \code{table\textunderscore options()}.  These
...
\begin{Schunk}
\begin{Sinput}
RowFactor(x, name, spacing=3, space=1, nopagebreak="\\nopagebreak")
\end{Sinput}
\end{Schunk}
...
\item[spacing] Extra spacing is added after every group of \code{spacing} lines.
...
I gratefully acknowledge helpful suggestions and hints from Rich Heiberger and Frank Harrell.
