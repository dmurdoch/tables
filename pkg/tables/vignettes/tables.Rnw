...
There is also the \code{html.tabular} method for the \code{Hmisc::html()} generic;
it produces output in HTML format.  Finally, see section \ref{sec:csv} for other 
output formats.
...
There's an optional third argument, which must be either \code{TRUE}
...
The \code{Percent()} pseudo-function is used to specify a statistic that depends on other values in the table.  It has two optional arguments:
\begin{description}
\item[\code{denom="all"}]  This specifies how the denominator
(argument \code{y} to \code{fn} below) is set.  Possible values are
...
values in the current column are used.  The special syntax \code{Equal(...)}
will compute the values in \code{...} for every case, and use those which
evaluate to one of the same values as the current cell as the reference
set. 
...
These two examples are different ways of producing almost the same 
table:
\begin{center}
<<results=tex>>=
...
latex( tabular( (Factor(gear, "Gears") + 1)
          *((n=1) + Percent() 
            + (RowPct=Percent(Equal(gear))) 
            + (ColPct=Percent(Equal(carb)))) 
         ~ (Factor(carb, "Carburetors") + 1)
          *Format(digits=1), data=mtcars ) )
@
\end{center}
The \code{NaN} values are a result of limitations
of the \code{Equal} pseudo-function:   it is based on
the observed values in the cell, so empty cells don't determine
anything.  A simple workaround would be to use a function
that returned 0 whenever the length of \code{x} was zero.
