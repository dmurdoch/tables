...
print(x, ...)
latex(x, file="", justification="c", tabular="tabular", ...)
\end{Sinput}
\end{Schunk}
...
be \code{"longtable"} can be used here, though often those also need 
modifications within the table, so this may not be very useful.
\end{description}
...
into a format suitable for display in \LaTeX\ documents.
The arguments are:
\begin{description}
\item[chars]
A character vector of formatted numeric values.
\item[minus]
Whether to pad cases with no minus sign with spacing of the same width.  If \code{TRUE}
and some entries are negative, then all positive entries will be padded with a space of the
same width as the minus sign.
...
Currently several examples of formula functions are provided, and but not all are
particularly robust.  Users can provide their own as well.  
Such functions
...
Example:  Show only the numeric columns in the iris data.

\begin{center}
<<results=tex>>=
latex( tabular( Species ~ All(iris)*mean, data=iris) )
@
\end{center}
...
The single argument is the text to insert.  See section \ref{sec:RowFactor} for an example
using it.
...
\code{Literal()} call is necessary to get the headings to repeat on subsequent pages.

<<results=tex>>=
subset <- 1:50
latex( tabular( (Literal("\\endhead\\hline\\endfoot%") 
                 + RowFactor(subset, "", 5))  ~ 
