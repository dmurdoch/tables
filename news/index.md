...
- The fix for issue [\#30](https://github.com/dmurdoch/tables/issues/30)
  was incomplete for some reason.
- [`table_options()`](https://dmurdoch.github.io/tables/reference/table_options.md)
  now returns the value of options if called with a character argument.
- An option `escape` has been added to
  [`table_options()`](https://dmurdoch.github.io/tables/reference/table_options.md).
  If `TRUE`, any special characters in HTML or LaTeX output are escaped
...
- In a few places `len` was used instead of `length.out` in calls to
  [`rep()`](https://rdrr.io/r/base/rep.html), leading to warnings.
  Similarly, `rep` was used instead of `replace` in
  [`sample()`](https://rdrr.io/r/base/sample.html) (issue
  [\#28](https://github.com/dmurdoch/tables/issues/28)).
- `tabular` assumed it was on the search list, and didn’t work when
  called as
  [`tables::tabular`](https://dmurdoch.github.io/tables/reference/tabular.md)
  (issue [\#30](https://github.com/dmurdoch/tables/issues/30)).
...
- A factor level of `""` caused an error when displayed. It will now be
  changed to `" "` for display (issue
  [\#19](https://github.com/dmurdoch/tables/issues/19)).
- The `VignetteBuilder` field in `DESCRIPTION` has been changed to
  `rmarkdown` so that Pandoc can be found even when it is not on the
  `PATH`.
...
- [`PlusMinus()`](https://dmurdoch.github.io/tables/reference/PlusMinus.md)
  and [`Paste()`](https://dmurdoch.github.io/tables/reference/Paste.md)
  didn’t handle formatting properly when multiple columns were involved
  (issue [\#13](https://github.com/dmurdoch/tables/issues/13)).
- [`toLatex()`](https://dmurdoch.github.io/tables/reference/latex.tabular.md)
  with a non-empty `file` argument didn’t write to file properly.
  (Reported by Reinhard Kerschner and F. Hortner.)
- In some cases
  [`All()`](https://dmurdoch.github.io/tables/reference/All.md) would
  give an error (issue
  [\#17](https://github.com/dmurdoch/tables/issues/17)).
- [`toTinytable()`](https://dmurdoch.github.io/tables/reference/toTinytable.md)
  function has been added to work with the `tinytable` package.
...
- Add
  [`useGroupLabels()`](https://dmurdoch.github.io/tables/reference/useGroupLabels.md)
  function.
- Add
  [`HTMLfootnotes()`](https://dmurdoch.github.io/tables/reference/HTMLfootnotes.md)
  function.
- Add method for
...
- rbind() failed in a complicated example, because the classes or names
  of the column labels differed.
...
- Added … argument to toKable() so that options can be passed to control
  the conversion. (Thanks to Mary Putt for pointing out the need for
  this.)
- Fixed bug in as.tabular.data.frame() that failed if any columns were
  factors. (Reported by Daniel Farewell.)
...
- Added check for bad args to DropEmpty(), and fixed example which had
  them.
- Added “nearData” argument to Heading() etc, to fix Hline() bug
  reported by Johannes Ranke.
- Fixed bug in justification of row labels.
- Fixed bug when statistic had the wrong length, reported by Stefan
  Marr.
- Added toKable() function, for use with the kableExtra package.
- Added knit_print.tabular() method, so that tabular objects will print
  with correct formatting in knitr documents.
...
- Updated options for html.tabular to use better CSS and HTML5 in the
  tables.
...
- Edited messages to allow internationalization. No translations added
  so far.

## tables 0.7.87

- Added “append” argument to latex.tabular() and html.tabular(), and
  made explicit that connections are allowed for the “file” argument.
  (Suggestion of Marc Halperin.)

## tables 0.7.86

- Fixed scoping bug: locally defined format functions were not always
  found. (Reported by Doug Ezra Morrison.)

## tables 0.7.79

CRAN release: 2014-07-24

- Added Equal() and Unequal() pseudo-functions for more flexible
...
- The cbind() and rbind() methods now allow NULL in the list of
  arguments. (Suggestion of Jeff Newmiller.)
- Added a note in the vignette about how to use RowFactor with
  nopagebreak in a longtable environment. (Based on some sleuthing by
  Jeffrey Miller.)
- Added HTMLcaption to the table options to insert an HTML caption.
  (Suggestion of Joseph Larmarange.)
...
- If a summary doesn’t produce a scalar value, formatting messed up the
  display. Now a warning will be issued and that cell will be displayed
  as or similar.
...
- Missing values were not handled correctly when computing counts, and
  when working with factors. (Reported by Manuel Reif.)
...
- Added optional parameter to Heading() to use it only if another hasn’t
  already been set.
...
- Added levelnames argument to RowFactor and Factor to allow label
  customization.
- Added Multicolumn function.
- Added as.matrix(), write.csv.tabular() and write.table.tabular()
  (suggestions of Greg Snow).
- Added LaTeX escapes to text in the body or labels constructed by All()
  or the factor labels. (Suggestion of Dieter Menne.)
- Added a caption to the longtable example. (Suggestion of Dieter
  Menne.)
- Added the version number to the vignette title page.
- Fixed RowFactor to add space before first row, not after last row.
  (Bug report from Dieter Menne.)
...
- Gave different defaults for row label justification and data
  justification
